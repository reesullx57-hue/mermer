...
export default function FormTypesTab() {
  const [items, setItems] = useState<FormTypeWithAudit[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [formMode, setFormMode] = useState<FormMode>(null);
  const [selectedItem, setSelectedItem] = useState<FormTypeWithAudit | null>(null);
  ...
  async function loadItems() {
    ...
    setError(null);
    try {
      const data = await fetchFormTypes();
      setItems(data);
    } catch (err) {
      if (err instanceof CatalogApiError) {
        ...
        setError('Form tipleri yüklenirken bir hata oluştu.');
      }
  ...
  function openEditForm(item: FormTypeWithAudit) {
    setFormMode('edit');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (formMode === 'create') {
        ...
        showToast('Form tipi başarıyla oluşturuldu.', 'success');
      } else if (formMode === 'edit' && selectedItem) {
        const request: UpdateCatalogItemRequest = {
          nameTr: formData.nameTr,
          coefficient: formData.coefficient,
        };
        await updateFormType(selectedItem.id, request);
        showToast('Form tipi başarıyla güncellendi.', 'success');
      }
      closeForm();
      loadItems();
    } catch (err) {
  ...
  async function handleDeactivate(item: FormTypeWithAudit) {
    if (!confirm(`"${item.nameTr}" form tipini devre dışı bırakmak istediğinizden emin misiniz?`)) {
      return;
    }

    try {
      await deactivateFormType(item.id);
      showToast('Form tipi devre dışı bırakıldı.', 'success');
      loadItems();
    } catch (err) {
      if (err instanceof CatalogApiError) {
        if (err.code === 'FORBIDDEN') {
    ...
    <div>
      <div className="mb-6 flex justify-between items-center">
        <p className="text-gray-600">
          Tezgah form tipleri (L, U, düz vb.) ve fiyatlandırma katsayıları
        </p>
        <button
      ...
      {!loading && !error && items.length === 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Yeni bir form tipi oluşturarak başlayın.
          </p>
          <button
            onClick={openCreateForm}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">
                {formMode === 'create' ? 'Yeni Form Tipi' : 'Form Tipi Düzenle'}
              </h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="Örn: STRAIGHT, L_SHAPE, U_SHAPE"
              ...
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="Örn: Düz, L Form, U Form"
