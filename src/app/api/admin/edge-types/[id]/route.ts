 ...
 * Update an edge type (admin only)
 */
export async function PATCH(
  ...
  const validation = EdgeTypeUpdateSchema.safeParse(body);
  ...
  try {
    const edgeType = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.edgeType.update({
        where: { id: params.id },
        data: updateData,
      });
    ...
    if (error.message === 'NOT_FOUND') {
      return NextResponse.json(
        { error: { code: 'NOT_FOUND', message: 'Edge type not found' } },
        { status: 404 }
      );
    }
    throw error;
